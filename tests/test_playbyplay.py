...
class PlayByPlayTestCase(TestCase):
    ...
    def test_generate_seconds_remaining(self):
        test_xml = os.path.join(TEST_DATA_DIR, 'playbyplay_NFL.xml')
        fetcher = FilesystemFetcher(path=test_xml)
        plays = sportsdirect.playbyplay.get_plays(sport='football',
            league='NFL', season='2010-2011', competition=28192, fetcher=fetcher)
        play = next(p for p in plays
                    if p.play_id == '/sport/football/play-by-play/event:471')
        self.assertEqual(play.seconds_remaining_in_game, 2889)
        play = next(p for p in plays
                    if p.play_id == '/sport/football/play-by-play/event:516')
        self.assertEqual(play.seconds_remaining_in_game, 1977)
        play = next(p for p in plays
                    if p.play_id == '/sport/football/play-by-play/event:566')
        self.assertEqual(play.seconds_remaining_in_game, 1100)
        play = next(p for p in plays
                    if p.play_id == '/sport/football/play-by-play/event:636')
        self.assertEqual(play.seconds_remaining_in_game, 14)
